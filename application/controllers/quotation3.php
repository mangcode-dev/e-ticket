...
class Quotation3 extends CI_Controller {
	...
	public function add(){
		...
		if($action=='addQuotation'){
			...
			$p['contract'] = $this->input->post('contract');
			$p['dateStart'] = $this->input->post('dateStart');
			$p['dateEnd'] = $this->input->post('dateEnd');
			$p['txt_cust'] = $this->input->post('txt_cust');
			$p['custAddress'] = $this->input->post('custAddress');
			$p['numrow'] = $this->input->post('numrow');
			// var_dump($p);
			// exit();
			
			for ($i=1; $i <= $p['numrow']; $i++) { 
				$p['des'.$i] = $this->input->post('des'.$i);
				$p['quantity'.$i] = $this->input->post('quantity'.$i);
				$p['price'.$i] = $this->input->post('price'.$i);
				$p['total'.$i] = $this->input->post('total'.$i);
				$p['unit'.$i] = $this->input->post('unit'.$i);
				$p['totalall'.$i] = $this->input->post('totalall'.$i);

			}
			// $p['des1'] = $this->input->post('des1');
			$p['totalPrice'] = $this->input->post('totalPrice');
			$p['service_order'] = $this->input->post('txt_value1');
			$p['service_type'] = $this->input->post('txt_value2');
			$p['source'] = $this->input->post('txt_value3');
			$p['destination'] = $this->input->post('txt_value4');
			$p['delivery_time'] = $this->input->post('txt_value5');
			$p['condition_pay'] = $this->input->post('txt_value6');
			$p['service_time'] = $this->input->post('txt_value7');
			$p['policy'] = $this->input->post('txt_value8');
			$p['bidder1'] = $this->input->post('txt_value9');
			$p['bidder2'] = $this->input->post('txt_value10');
			...
			$this->form_validation->set_rules('txt_cust', 'txt_cust', 'trim|required');
			...
			if($this->form_validation->run() == FALSE){
				...
				$result = $this->backoffice_model->addQuotation($p);
		...
		$this->template->write_view('page_content', 'themes/'. $this->theme .'/view_addquotation3.php', $data);
	...
	public function searchCust(){
          ...
          if(isset($_GET['q'])){ //ตรวจสอบว่ามีการส่งค่า get มาหรือเปล่า
            ...
            $array = array('first_name' => $name, 'last_name' => $name);

            $this->db->like("first_name",$name)->or_like("last_name",$name)->select('*');
            $result=$this->db->from("customer")->get()->result();
          }
 
          foreach($result as $key){
                $arr[]=$key->first_name."  ".$key->last_name;//จากนั้นก็นำสีมา เข้ารหัสแบบ json
          }
   ...
   public function getAddress(){
          ...
          if(isset($_GET['q'])){ //ตรวจสอบว่ามีการส่งค่า get มาหรือเปล่า
            ...
            $this->db->select('*')->where("first_name =",$name); //ให้แสดงชื่อสีอะไรก็ตาม ที่มีคำคล้ายกับตัวอักษรที่ส่งมา
            $result=$this->db->from("customer")->get()->result();
          }
 
          foreach($result as $key){
                $arr[]=$key->address;//จากนั้นก็นำสีมา เข้ารหัสแบบ json
          }
          echo $_GET['callback']."(".json_encode($arr).")";   //แล้วก็ส่งค่ามันกลับไป 
